...
namespace pokemon {
    ...
    void pictures_command::write(Server& server, PROTOCOL protocol, std::shared_ptr<sockpp::tcp_socket> socket) {
        if (!socket || !(*socket)) {
            return;
        }

        std::string msg = get_pictures_to_send(server.get_images_repository().get_images());
