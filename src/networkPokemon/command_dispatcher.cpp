...
namespace pokemon {
    ...
    void command_dispatcher::dispatch_write(Server& server, PROTOCOL protocol, std::shared_ptr<sockpp::tcp_socket> socket) {
        auto it = m_commands.find(protocol);
        if (it != m_commands.end()) {
            it->second->write(server, protocol, std::move(socket));
        }
    }
